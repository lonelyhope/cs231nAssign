...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    pass
    H = W1.shape[1]
    rh = np.dot(X, W1) + b1.reshape(1, H)
    h = rh.copy()
    h[h < 0] = 0
    C = W2.shape[1]
    scores = np.dot(h, W2) + b2.reshape(1, C)
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    ...
    loss = np.sum(np.log(sum_exp_score))
    sy = scores[range(N), y]
    loss -= np.sum(sy)
    ...
    #############################################################################
    pass
    P = np.exp(scores) / sum_exp_score.reshape(N, 1)
    ds = P
    ds[range(N), y] -= 1 # dScores N * C
    ds /= N
  ...
  def loss0(self, X, y=None, reg=0.0):
    """
    Compute the loss and gradients for a two layer fully connected neural
    network.
    Inputs:
